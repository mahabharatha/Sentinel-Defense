# Sentinel Defense — Repository Structure
...
the root, runtime artifacts under `data/`, documentation under `docs/`, and
a small set of operator scripts under `scripts/`. There is no `src/` indirection.

> **Conventions.** Italics denote regenerable / runtime artifacts that are
...
│
└─ data/                        # *Runtime artifacts. Mostly ignored by git.*
   ...
   ├─ art_runs/                 # *IBM ART native artifacts*
   ├─ foolbox_runs/             # *Foolbox native artifacts*
   ├─ garak_runs/               # *Garak native artifacts*
   ├─ pyrit_runs/               # *PyRIT native artifacts (per attack run)*
   ├─ textattack_runs/          # *TextAttack native artifacts*
...
`reconcile_job_statuses`. Centralizes path resolution to keep artifacts
...
- **`normalized.py`** — the unified severity engine. Constants
  ...
  HTML rendering, artifact writers.
...
4. Live Job Status (with Original + Normalized Artifacts side-by-side beneath)
...
│
├─ art_runs/<job_id>/reports/    # IBM ART native artifacts
...
Why this layout: every framework writes its native artifacts into its own
...
- **Per-job runtime artifacts** under `data/jobs/`, `data/job_reports/`,
