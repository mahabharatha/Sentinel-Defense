...
class TrOCRSmallArtAdapter(BaseScanAdapter):
    """Real ART-backed OCR adapter using TrOCR small printed."""
    ...
    def capabilities(self) -> WrapperCapabilities:
        return WrapperCapabilities(
            ...
            notes="Pinned reference wrapper for the shared IBM ART OCR harness using microsoft/trocr-small-printed.",
        )
    ...
    def validate_config(self, config: dict[str, Any]) -> list[str]:
        ...
        if configuration.get("execution_backend") != "python_process_wrapped":
            errors.append("TrOCRSmallArtAdapter requires the python_process_wrapped backend.")
        if model.get("modality") != "vision":
            errors.append("TrOCRSmallArtAdapter requires model modality 'vision'.")
        if model.get("task_family") != "ocr":
            errors.append("TrOCRSmallArtAdapter requires task family 'ocr'.")
        if "art" not in frameworks:
            errors.append("TrOCRSmallArtAdapter requires the IBM ART framework to be selected.")
        if not sample_path:
        ...
        if "whitebox" not in set(configuration.get("scan_modes", [])):
            errors.append("TrOCRSmallArtAdapter is intended for a real white-box ART run; enable whitebox mode.")
        return errors
    ...
    def run_framework_scan(self, framework: str, config: dict[str, Any]) -> dict[str, Any]:
        if framework != "art":
            raise NotImplementedError(f"TrOCRSmallArtAdapter does not implement framework '{framework}'.")
